...
const HabitProgressChart = ({ progressHistory }) => {
  const [chartData, setChartData] = useState([]);

  useEffect(() => {
    if (progressHistory && progressHistory.length > 0) {
      // Process the data for display
      const processed = progressHistory.map(entry => ({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg">
      <h3 className="text-lg font-medium mb-4 text-gray-800 dark:text-white">Progress History</h3>
